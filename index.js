    ...
    try {
        ...
        for (const match of matches) {
            ...
            if (!matchTime.isValid()) {
                console.warn(
                    `Invalid match.time for match ${match._id}. Skipping...`
                );
                continue;
            }

            if (
                matchTime.isBefore(now, "minute") &&
                matchTime.isSame(now, "day")
            ) {
            ...
            if (timeDifference > 15 * 60 * 1000) {
                ...
                timeDifference <= 0 &&
                Math.abs(timeDifference) <= 2 * 60 * 60 * 1000
            ) {
                newStatus = "جارية";
            ...
            if (match.status !== newStatus) {
                ...
                await match
                    .save()
                    .then(() =>
                        console.log(`Match ${match._id} saved successfully.`)
                    )
                    .catch(err =>
                        console.error(
                            `Error saving match ${match._id}:`,
                            err.message
                        )
                    );
            }
